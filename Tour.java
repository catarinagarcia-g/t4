...
public class Tour {
    ...
    public void insertNearest(Point p) {
        ...
        // 1. Encontrar o nó do tour mais próximo do ponto p
        Node current = start;
        do {
        ...
        // 2. Inserir o novo nó logo após o nearestNode
    ...
    public void insertSmallest(Point p) {
        ...
        Node bestNode = start;
        double minExtraCost = Double.MAX_VALUE;

        // 1. Percorrer cada aresta (current -> current.next)
        Node current = start;
        do {
            double currentDist = current.p.distanceTo(current.next.p);
            double distParaP = current.p.distanceTo(p);
            double distDePParaNext = p.distanceTo(current.next.p);
            
            // Custo extra de desviar o caminho para passar por p
            double extraCost = distParaP + distDePParaNext - currentDist;
        ...
        // 2. Inserir o ponto na posição que causou o menor aumento
        Node newNode = new Node();
        newNode.p = p;
